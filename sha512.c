...
// circular right shift of the 64-bit argument x by n bits
...
#define ROUND(a, b, c, d, e, f, g, h, k, w) { \
	uint64_t tmp0 = h + T1(e) + CH(e, f, g) + k + t; \
...
void sha512_chunk(sha512_ctx *ctx, const uint8_t chunk[128])
{
	...
	for (i = 0; i < 16; i++)
		w[i] = GET_UINT64(&chunk[8*i]); // 64-bit value
	...
	// The first 16 values of Wt are taken directly from the 16 words of the current work
	for (i = 0; i < 16; i += 8) {
...
void sha512_process(sha512_ctx *ctx, const uint8_t *data, size_t length)
{
	...
	if (ctx->buflen != 0 && ctx->buflen + length >= 128) {
		...
		// copy the message length to the ctx->buffer
	...
	while (length > 128) {
		// Process message in 1024-bit(128 byte) block
		sha512_chunk(ctx, data);
		data += 128;
		length -= 128;
	}

	// the remaining data, less than 128 byte
	if (length) {
		// copy to the ctx->buffer
...
void sha512_final(sha512_ctx *ctx, uint8_t out[64])
{
	...
	// need padding the fill array (just need flen  size of fill array)
	uint32_t flen = (ctx->buflen < 112) ? 112-ctx->buflen : 240-ctx->buflen;
	// then add the message length	
	uint8_t buf[16];
	...
	// copy the flen size of fill array to ctx->buffer
